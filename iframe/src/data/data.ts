import { DataObject, InitialAppData, LastEntryData } from "../types/dataObject";
import {
  GetMemoParams,
  PostMemoParams,
...
} from "../types/saveProgress";

async function save_html_progress(param: SaveHtmlProgressParam) {
  console.log("save_html_progress", param);
}

async function save_video_progress(
  param: SaveVideoProgressParamForUserCallback
) {
  console.log("save_video_progress", param);
}

async function save_quiz_progress(param: SaveQuizProgressParam) {
  console.log("save_quiz_progress", param);
}

async function get_memo(param: GetMemoParams) {
  console.log("getMemoByEntryId", param);
  return `memo from callback entry id: ${param.id} group id: ${param.group_id}`;
}

async function post_memo(param: PostMemoParams) {
  console.log("postMemo", param);
}

async function post_question_with_files(param: PostQuestionWithFilesParams) {
  console.log("save_quiz_progress", param);
}

async function save_last_entry_data(param: LastEntryData): Promise<void> {
  console.log("save_last_entry_data", param);
}

export const initialAppData: InitialAppData = {
  id: 1, // 앱 seq_id
  title: "[상시] 미디어 크리에이터 교육과정 수업이름이 길어지면", // 통합 학습 제목
  logo: "/img/logo.png", // 사이트 로고
  error_url: "https://www.step.or.kr/usrs/faq/UsrsFaqListForm.do", // lms 오류 신고 페이지로 이동
  hidden_frame_url: "/hidden-frame.html", // hidden frame url
  is_sequence: false,
  ...
  entries: [
    {
      id: 1, // 회차 seq_id, group_id
      title: "1. 유튜브 계획하기", // 회차 제목
      is_next: true, // 다음 페이지 여부
      entries: [
        {
          id: 1, // 강의 seq_id, page_id
          group_id: 1, // 회차 seq_id,
          title: "1-0. 퀴즈",
        ...
        },
        {
          ...
          is_next: true, // 다음 페이지 여부
          datas: {
            type: "html_string", // 콘텐츠 타입
            html: "<h2>h2 tag</h2><span>test html</span>",
            time_to_complete: 10,
...
export const DATA_OBJECT: DataObject = {
  callbacks: {
    save_html_progress,
    save_video_progress,
    save_quiz_progress,
